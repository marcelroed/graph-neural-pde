...
def print_image_T(model, dataset, opt, modelpath, height=2, width=3):
    ...
    for i in range(height * width):
        ...
        if opt['im_dataset'] == 'MNIST':
            plt.imshow(batch.x[torch.nonzero(mask)].view(model.opt['im_height'], model.opt['im_width']), cmap='gray',
                       interpolation='none')
...
def print_image_path(model, dataset, opt, height, width, frames):
    ...
    for i in range(height * width):
        ...
        mask = batch.batch == i
        if opt['im_dataset'] == 'MNIST':
            plt.imshow(paths[i, 0, :].view(model.opt['im_height'], model.opt['im_width']), cmap='gray',
                       interpolation='none')
        elif opt['im_dataset'] == 'CIFAR':
    ...
    def update(ii):
        for i in range(height * width):
            ...
            plt.tight_layout()
            if opt['im_dataset'] == 'MNIST':
                plt.imshow(paths[i, ii, :].view(model.opt['im_height'], model.opt['im_width']), cmap='gray',
                           interpolation='none')
            elif opt['im_dataset'] == 'CIFAR':
