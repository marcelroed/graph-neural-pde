...
def batch_animation(paths, labels, frames, fps, opt, pic_folder, samples):
    ...
    for i in range(samples):
        ...
        animation.save(f'{savefolder}/animation{i}.gif',
...
def model_comparison(model_keys, model_epochs, times, sample_name, samples, Tmultiple, partitions, batch_num):
    ...
    for sample in range(samples):
        ...
        for i, model_key in enumerate(model_keys):
            ...
            train_maski = train_mask[sample * (opt['im_height'] * opt['im_width']):(sample + 1) * (
                        opt['im_height'] * opt['im_width'])].long()
            mask = paths[sample, 0, :].reshape(-1, opt['im_height'] * opt['im_width'],
                                               opt['im_chan']) * train_maski.reshape(-1, 1)
