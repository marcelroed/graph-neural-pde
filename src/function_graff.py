...
class ODEFuncGraff(ODEFunc):
    ...
    def __init__(self, in_features, out_features, opt, data, device):
        ...
        self.deg_inv_sqrt = self.get_deg_inv_sqrt(data).to(
            device)  # sending this to device because at initi, data is not yet sent to device
        ...
        if self.opt['w_style'] in ['sum', 'prod', 'neg_prod']:
            ...
            if self.opt['w_diag_init'] == 'linear':
                ...
                if self.time_dep_w or self.time_dep_struct_w:
                    ...
                    self.W_D = Parameter(self.opt['w_diag_init_q'] * d_range / (d - 1) + self.opt['w_diag_init_r'],
                                         requires_grad=opt['w_param_free'])
            else:
                if self.time_dep_w or self.time_dep_struct_w:
    ...
    def set_W(self, T=None):
        ...
        if self.opt['w_style'] in ['prod']:
            ...
            if self.time_dep_w:
                ...
                W_sum = W + self.at[T] * F.tanh(self.bt[T] * T + self.gt[T]) * torch.eye(n=W.shape[0], m=W.shape[1],
                                                                                         device=self.device)
            else:
