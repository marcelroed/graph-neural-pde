...
class MixedODEblock(ODEblock):
    def __init__(self, odefunc, regularization_fns, opt, num_nodes, edge_index, edge_attr, device,
                 t=torch.tensor([0, 1]), gamma=0):
        super(MixedODEblock, self).__init__(odefunc, regularization_fns, opt, t)

        self.odefunc = odefunc(self.aug_dim * opt['hidden_dim'], self.aug_dim * opt['hidden_dim'], opt, edge_index,
                               edge_attr, device)
