...
class GNN_image_pixel(BaseGNN):
    def __init__(self, opt, num_features, num_nodes, num_classes, edge_index, edge_attr=None,
        ...
        self.odeblock = block(self.f, self.regularization_fns, opt, num_nodes, edge_index, edge_attr, device,
    ...
    def forward_plot_path(self, x, frames):  # stitch together ODE integrations
        ...
        for f in range(frames):
            ...
            if self.eval:  # undo batch norm
                ...
                path = (z - self.bn.bias) * (
                            self.bn.running_var + self.bn.eps) ** 0.5 / self.bn.weight + self.bn.running_mean
    ...
    def forward_plot_SuperPix(self, x, frames):  # stitch together ODE integrations
        ...
        if self.opt['function'] == 'transformer':
            atts = [
                self.odeblock.odefunc.edge_weight]  # not calculated until 1st forward pass attention_weights]  # edge_weight]
        ...
        paths = [
            x]  # [torch.sigmoid(self.m2(x))] #[x]#.view(-1, self.opt['im_width'] * self.opt['im_height'] * self.opt['im_chan'])]
